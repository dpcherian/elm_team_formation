...
def write_csv(names,groups,output_file):
    ...
    with open(fileName, 'w', newline='') as file:
        ...
        writer.writerow(["Name","Domain 1","Domain 2","Domain 3"])
        ...
        for group in groups:
            ...
            for person in group:
                ...
                line = [names[index], d1[index], d2[index], d3[index], d4[index], d5[index]]
...
# constraints = np.array([d1,d2,d3,d4,d5])
...
def run_indeterminate_groups(names,preferred_groupmate,constraints,max_groups,output_filename):
    ...
    print("Done pairing. Creating output CSV.")
...
#         d1 = data[c1.get()].values
#         d2 = data[c2.get()].values
#         d3 = data[c3.get()].values

#         d4 = data[c4.get()].values
#         d5 = data[c5.get()].values
...
#Label 1
label1 = Label(root,text = 'Raw Data File (example.csv)')
label1.pack(pady=5)
label1.config(justify = CENTER)

fileName = Entry(root, width = 50)
fileName.insert(END,'Biased_Data.csv')
